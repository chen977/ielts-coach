...
type Filter = 'all' | 'speaking' | 'listening' | 'writing'
...
export default function HistoryClient({ items }: HistoryClientProps) {
  ...
  const filters: { id: Filter; label: string; count: number }[] = [
    ...
    { id: 'writing', label: 'Writing', count: items.filter(i => i.type === 'writing').length },
  ]
        ...
        {filtered.map(item => (
          <div key={item.id} className="bg-white rounded-2xl border border-gray-100 p-5">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${item.type === 'speaking' ? 'bg-sky-50' : item.type === 'writing' ? 'bg-amber-50' : 'bg-emerald-50'}`}>
                {item.type === 'speaking' ? (
            ...
            {item.type === 'writing' && item.scores && (item.scores as { criteria?: { criterion: string; band: number }[] }).criteria && (
              <div className="mt-3 pt-3 border-t border-gray-50 flex gap-3 flex-wrap">
                {((item.scores as { criteria: { criterion: string; band: number }[] }).criteria).map(c => (
                  <div key={c.criterion} className="text-xs">
                    <span className="text-gray-400">{c.criterion}: </span>
